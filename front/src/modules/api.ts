...
export class Module {
  public readonly ssr = import.meta.env.SSR;
  public readonly state = reactive({ 
    isLogged: !this.ssr && !!localStorage.auth_token,
  });
  protected _token: string | null = this.ssr ? null : localStorage.auth_token || null;
  ...
  protected get root(): string {
    const apiUrlSsr = this.ssr ? process.env['VITE_API_URL'] : undefined;
    const result = apiUrlSsr || import.meta.env.VITE_API_URL;
  ...
  public async authSendCode(phone: string, captcha: string | null = null): Promise<Retry> {
    //await new Promise(resolve => setTimeout(resolve, 3000));
    const result = await axios.post(this.root, { method: 'user.sendSmsCode', params: { phone, captcha } });
